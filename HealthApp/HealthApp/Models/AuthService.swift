...
class AuthService {
    ...
    func register(email: String, password: String, doctor: Doctor, onComplete: Completion?) {
        Auth.auth().createUser(withEmail: email, password: password) { (result, error) in
            if let error  = (error as NSError?) {
                ...
                if result?.user.uid != nil {
                    ...
                    finalDoctor.uid = (result?.user.uid)!
                    DatabaseService.shared.saveDoctor(doctor: finalDoctor)
                    do {
                        try self.realm?.write {
                            self.realm?.add(finalDoctor)
                        }
                    } catch {
