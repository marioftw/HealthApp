...
class EditProfileViewController: UIViewController {
    ...
    var specialityTextField: UITextField!
   ...
   override func viewDidLoad() {
    ...
    @IBAction func saveButtonPressed(_ sender: UIBarButtonItem) {
        if firstNameTextField.text != "", lastNameTextField.text != "", specialityTextField.text != "", phoneTextField.text != nil, addressTextField.text != nil, profileImageView.image != nil {
            do {
                try realm?.write {
                    doctor?.firstName = firstNameTextField.text!
                    doctor?.lastName = lastNameTextField.text!
                    doctor?.dataProfileImage = profileImageView.image?.pngData()
                }
            } catch {
            ...
            //doctor?.saveBasicInfoInFirebase()
    ...
    func saveProfileImageInFirebase() {
        if doctor != nil {
            ...
            var imageData: Data?
            imageData = doctor?.dataProfileImage ?? profileImageView.image?.pngData()
            
            if let data = imageData {
                let imageName = "Profile\(doctorUID).jpg"
                let ref = DatabaseService.shared.imageStorageRef.child("Doctor").child("ProfilePicture").child(imageName)
                _ = ref.putData(data, metadata: nil, completion: { (metadata, error) in
                    if error != nil {
                        ...
                        ref.downloadURL(completion: { (url, error) in
                            ...
                            DatabaseService.shared.savePictureRef(uid: doctorUID, url: url!)
                        })
                    }
                })
...
extension EditProfileViewController: UITableViewDelegate, UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            ...
            if let image = doctor?.profilePicture {
            ...
            case 1:
                ...
                cell.textField.placeholder = doctor?.firstName ?? ""
            case 2:
                self.lastNameTextField = cell.textField
                cell.label.text = "Last Name"
                cell.textField.placeholder = doctor?.lastName ?? ""
            case 3:
                self.specialityTextField = cell.textField
                cell.label.text = "Speciality"
                cell.textField.placeholder = doctor?.specialty ?? ""
            case 4:
                self.phoneTextField = cell.textField
                cell.label.text = "Phone"
                cell.textField.placeholder = doctor?.phone ?? ""
            default:
                self.phoneTextField = cell.textField
                cell.label.text = "Address"
                cell.textField.placeholder = doctor?.direction ?? ""
            }
